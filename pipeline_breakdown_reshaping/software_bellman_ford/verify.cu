...
int nds_bfs(struct resources *res, uint64_t id, uint64_t num_of_vertices, uint64_t num_of_subvertices) {
    ...
    col_fetch_timing = timing_info_new(total_iteration * (num_of_vertices / num_of_subvertices));
    if (col_fetch_timing == NULL) {
        printf("cannot create col_fetch_timing\n");
        return -1;
    }
    
    reshape_timing = timing_info_new(total_iteration * (num_of_vertices / num_of_subvertices));
    ...
    do {
        ...
        for (st = 0; st < num_of_vertices / num_of_subvertices; st++) {
            ...
            for (sub_st = 0; sub_st < num_of_vertices / num_of_subvertices; sub_st++) {
