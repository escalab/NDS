...
int nds_knn(struct resources *res, uint64_t id, uint64_t ref_nb, uint64_t subref_nb, const double *query, uint64_t query_nb, uint64_t k) {
    ...
    size_t dim = ref_nb;
    err0 = cudaMalloc((void**) &ref_dev, SUB_M * sizeof(double) * dim); // 4 GB
    ...
    col_fetch_timing = timing_info_new(total_iteration * (ref_nb / subref_nb));
    if (col_fetch_timing == NULL) {
        printf("cannot create col_fetch_timing\n");
        return -1;
    }

    reshape_timing = timing_info_new(total_iteration * (ref_nb / subref_nb));
    ...
    double *src_ptr;
    // blockGEMM
    for (i = 0, st = 0; i < ref_nb; i+=SUB_M, st++) {
        ...
        cudaMemcpy(ref_dev, reshaped_data, M * SUB_M * sizeof(double), cudaMemcpyHostToDevice);
        ...
        compute_squared_norm<<<(SUB_M+THREADS_PER_BLOCK-1)/THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(ref_dev, SUB_M, ref_pitch, dim, ref_norm_dev);
        if (cudaGetLastError() != cudaSuccess) {
            printf("ERROR: Unable to execute kernel\n");
            free(knn_dist);
            free(knn_index);
        ...
        stat = cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, query_pitch, ref_pitch, dim, &alpha, query_dev, query_pitch, ref_dev, ref_pitch, &beta, dist_dev + i*query_nb, query_pitch);
