...
void tensor_reshape(double *dst, double *src, size_t m, size_t sub_m) {
...
int nds_ttv(struct resources *res, uint64_t id, uint64_t size, uint64_t sub_size, const double *A, double *C) {
    ...
    fetch_timing = timing_info_new(total_iteration * SUB_M);
    if (fetch_timing == NULL) {
        printf("cannot create fetch_timing\n");
        return -1;
    }

    reshape_timing = timing_info_new(total_iteration * SUB_M);
    ...
    double *reshape_ptr;
    double *src_ptr;
    double *reshaped_data = (double *) calloc(SUB_M * SUB_M * SUB_M, sizeof(double));
    uint64_t z;
    cudaError_t err;
    ...
    for (n = 0; n < N; n+=SUB_M) {
        for (m = 0; m < M; m+=SUB_M) { 
            ...
            for (k = 0; k < K; k+=SUB_M) {
                // 512 * 512 * 512
                reshape_ptr = reshaped_data;
                for (z = k; z < k+SUB_M; z++) {
                    timing_info_push_start(fetch_timing);
                    sock_write_request(res->req_sock, id, n/SUB_M, m/SUB_M, z, 1, 0);
                    ...
                    tensor_reshape(reshape_ptr, src_ptr + n * M + m, M, SUB_M);
                    if (sock_write_data(res->sock)) { /* just send a dummy char back and forth */
                        fprintf(stderr, "sync error before RDMA ops\n");
                        return -1;
                    }
                    free(resp);
                    reshape_ptr += SUB_M * SUB_M;
                    timing_info_push_end(reshape_timing);
                }

                timing_info_push_start(copy_in_B_timing);
                cudaMemcpy(d_B, reshaped_data, AGGREGATED_SZ, cudaMemcpyHostToDevice);
                ...
                timing_info_push_end(copy_in_B_timing);
