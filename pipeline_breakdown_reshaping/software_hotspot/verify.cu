...
void submatrix_reshape(double *dst, double *src, size_t m, size_t sub_m) {
...
__host__ int spdk_nds_hotspot(struct resources *res, uint64_t id, uint64_t m, uint64_t sub_m) {
    ...
    reshape_timing = timing_info_new(dsize * 2);
    ...
    for (t = 0; t < total_iterations; t+=num_iterations) {
        for (i = 0; i < m / sub_m; i++) {
            for (j = 0; j < m / sub_m; j++) {
                ...
                sock_write_request(res->req_sock, id, i, i+1, SUB_M, 2, 0);
                ...
                submatrix_reshape(reshaped_data, src_ptr + j * SUB_M, M, SUB_M);
                if (sock_write_data(res->sock)) { /* just send a dummy char back and forth */
                    fprintf(stderr, "sync error before RDMA ops\n");
                    return -1;
                }
                ...
                cudaMemcpy(MatrixTempIn, reshaped_data, dsize * sizeof(double), cudaMemcpyHostToDevice);
                ...
                timing_info_push_start(copy_in_timing);
                cudaMemcpy(MatrixPower, reshaped_data, dsize * sizeof(double), cudaMemcpyHostToDevice);
                timing_info_push_end(copy_in_timing);
