...
void rowstripe_reassemble(double *dst, double *src, size_t m, size_t sub_m, size_t granularity) {
    uint64_t chunk, i, row, col;
    uint64_t dsize = m * sub_m;
...
int nds_pagerank(struct resources *res, uint64_t id, uint64_t m, uint64_t sub_m) {
    ...
    col_reshape_timing = timing_info_new(total_iteration * (m / sub_m));
    ...
    for (i = 0; i < NITERS; i++) {
        ...
        for (st = 0; st < m / sub_m; st++) {
            ...
            cudaMemcpy(outedges, res->buf + resp->offset, M * SUB_M * sizeof(double), cudaMemcpyHostToDevice);
