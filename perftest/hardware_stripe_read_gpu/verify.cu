...
int nds_col_stripe_read(struct resources *res, uint64_t id) {
    ...
    cudaMalloc(&d_addr, M * SUB_M * sizeof(double));
    ...
    for (i = 0; i < NITERS; i++) {
        for (st = 0; st < (M / SUB_M); st++) {
            ...
            cudaMemcpy(d_addr, res->buf + entry->resp->offset, M * SUB_M * sizeof(double), cudaMemcpyHostToDevice);
...
int main(int argc, char *argv[]) {
    ...
    rc = nds_col_stripe_read(&res, matrix_id);
