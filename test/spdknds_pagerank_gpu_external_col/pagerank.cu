...
__global__ void pagerank_update(double *vertices, size_t st, double *in_graph, double *out_graph, int *updated, size_t num_of_vertices, size_t num_of_subvertices, int iter, int niters) {
    ...
    inedge = in_graph + id * num_of_vertices;
    outedge = out_graph + id;
    id = st + id;
    ...
    if (iter == 0) {
        for (i = 0; i < num_of_vertices; i++) {
            if (i != id && outedge[i * num_of_subvertices] != 0) {
                outedge[i * num_of_subvertices] = 1.0 / outc;
                *updated = 1;
            }
        }
        ...
        if (outc > 0) {
            ...
            for (i = 0; i < num_of_vertices; i++) {
                if (i != id && outedge[i * num_of_subvertices] != 0) {
                    outedge[i * num_of_subvertices] = pagerankcont;
                    *updated = 1;
                }
            }
        } 
 ...
 int main(int argc, char** argv) {
    ...
    for (iter = 0; iter < niters; iter++) {
        ...
        for (st = 0; st < (num_of_vertices / num_of_subvertices); st++) {
            ...
            return_size = tensorstore_get_row_stripe_submatrix(&client, fd_in, st, st+1, num_of_subvertices);
            ...
            gettimeofday(&h_start, NULL);
            return_size = tensorstore_get_col_stripe_submatrix(&client, fd_out, st, st+1, num_of_subvertices);
            ...
            if (updated_h) {
                ...
                return_size = tensorstore_write_col_stripe_submatrix(&client, fd_out, st, st+1, num_of_subvertices);
                gettimeofday(&h_end, NULL);
