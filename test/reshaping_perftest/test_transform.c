...
void request_submatrix(double *seq_matrix, double *tensor_matrix, uint64_t x, uint64_t y, uint64_t m, uint64_t sub_m) {
    uint64_t i, offset;
...
int main(int argc, char** argv) {
    ...
    tensor_matrix = (double *) calloc(sub_m * sub_m, sizeof(double));
    ...
    for (i = 0; i < m / sub_m; i++) {
        for (j = 0; j < m / sub_m; j++) {
            request_submatrix(seq_matrix, tensor_matrix, j, i, m, sub_m);
        }
    }
