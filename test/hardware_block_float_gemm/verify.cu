...
int spdk_nds_blockSgemm_half_pthread(struct resources *res, uint64_t id, uint64_t m, uint64_t sub_m, float *c) {
    ...
    cudaMalloc((void **) &a_sub_h, sizeof(float) * dsize);
    cudaMalloc((void **) &b_sub_h, sizeof(float) * dsize);
                ...
                for (sub_i = 0; sub_i < SUB_M; sub_i+=SUBSUB_M) {
                    for (sub_j = 0; sub_j < SUB_M; sub_j+=SUBSUB_M) {
                        for (sub_k = 0; sub_k < SUB_M; sub_k+=SUBSUB_M) {
                            cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, SUBSUB_M, SUBSUB_M, SUBSUB_M, &alpha, (b_sub_h + sub_k * SUB_M + sub_i), CUDA_R_32F, SUBSUB_M, (a_sub_h + sub_i * SUB_M + sub_k), CUDA_R_32F, SUBSUB_M, &beta, c_sub_f + sub_i * SUB_M + sub_j, CUDA_R_32F, SUBSUB_M, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
                        }
                    }
                }
