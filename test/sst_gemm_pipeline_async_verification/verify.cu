        ...
        for (j = 0; j < conf->m / conf->sub_m; j++) {
            for (k = 0; k < conf->m / conf->sub_m; k+=IO_QUEUE_SZ) {
                ...
                for (count = 0; count < IO_QUEUE_SZ; count++) {
                    ...
                    cudaMemcpyAsync(ptr_a, hugepage_ptr, dsize * sizeof(double), cudaMemcpyHostToDevice, conf->stream[count]);
    
                    // printf("fetching B[%lu][%lu]\n", count+k, j);
                    ptr_b = conf->b_sub_d + dsize * count;
                    hugepage_ptr = conf->hugepage_addr + (count * 2 + 1) * dsize * sizeof(double);
                    cudaMemcpyAsync(ptr_b, hugepage_ptr, dsize * sizeof(double), cudaMemcpyHostToDevice, conf->stream[count]);
...
int spdk_nds_blockSgemm_half_pthread(struct resources *res, uint64_t m, uint64_t sub_m, float *c) {
    ...
    size_t dsize;
    cublasHandle_t handle[IO_QUEUE_SZ];
    cudaStream_t stream[IO_QUEUE_SZ];
    ...
    struct timing_info *reduction_timing;
    ...
    reduction_timing = timing_info_new(dsize);
    if (reduction_timing == NULL) {
        printf("cannot create reduction_timing\n");
        return -1;
    }
    ...
    cudaMalloc((void **) &a_sub_h, sizeof(half) * dsize * IO_QUEUE_SZ);
    cudaMalloc((void **) &b_sub_h, sizeof(half) * dsize * IO_QUEUE_SZ);
    cudaMalloc((void **) &c_sub_f, sizeof(float) * dsize * IO_QUEUE_SZ);
    ...
    struct fifo_entry *entry = NULL;
    // blockGEMM
    dim3 gridSize((sub_m+32-1)/32, (sub_m+32-1)/32);
    dim3 blockSize(32, 32);
    for (i = 0; i < m / sub_m; i++) {
        for (j = 0; j < m / sub_m; j++) {
            // printf("working on C[%lu][%lu]\n", i, j);
            cudaMemset(c_sub_f, 0, sizeof(float) * dsize * IO_QUEUE_SZ);
            for (k = 0; k < m / sub_m; k+=IO_QUEUE_SZ) {
                for (count = 0; count < IO_QUEUE_SZ; count++) {
                    ...
                    ptr_a = a_sub_h + dsize * count;
                    ptr_b = b_sub_h + dsize * count;
                    d2h_kernel<<<(dsize+MAX_THREAD-1)/MAX_THREAD,MAX_THREAD, 0, stream[count]>>>(entry->a_sub_d, ptr_a, dsize);
                    d2h_kernel<<<(dsize+MAX_THREAD-1)/MAX_THREAD,MAX_THREAD, 0, stream[count]>>>(entry->b_sub_d, ptr_b, dsize);
                    free(entry);
                    timing_info_push_end(d2h_timing);
    
                    // gemm
                    timing_info_push_start(gemm_timing);
                    ptr_c = c_sub_f + dsize * count;
                    cublasGemmEx(handle[count], CUBLAS_OP_N, CUBLAS_OP_N, sub_m, sub_m, sub_m, &alpha, ptr_b, CUDA_R_16F, sub_m, ptr_a, CUDA_R_16F, sub_m, &beta, ptr_c, CUDA_R_32F, sub_m, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
                    timing_info_push_end(gemm_timing);    
                }
            }
            timing_info_push_start(reduction_timing);
            cudaDeviceSynchronize();
            if (IO_QUEUE_SZ > 1) {
                reduction_kernel_2D<<<gridSize, blockSize>>>(c_sub_f, sub_m, sub_m, IO_QUEUE_SZ);
            }
            timing_info_push_end(reduction_timing);
    ...
    printf("reduction time: %f ms\n", (float) timing_info_duration(reduction_timing) / 1000);
    ...
    tss = timing_info_get_timestamps(reduction_timing);
    fptr = fopen("reduction_timing.bin", "wb");
    fwrite(&tss->count, sizeof(uint64_t), 1, fptr);
    fwrite(tss->timestamps, sizeof(uint64_t), tss->count * 2, fptr);
    fclose(fptr);
    timing_info_free_timestamps(tss);    
    timing_info_free(reduction_timing);
