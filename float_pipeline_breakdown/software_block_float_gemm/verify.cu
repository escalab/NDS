...
int spdk_nds_blockSgemm_half_pthread(struct resources *res, uint64_t id, uint64_t m, uint64_t sub_m, float *c) {
    ...
    f_conf.m = m;
    f_conf.sub_m = sub_m;
    f_conf.a_sub_f = a_sub_f;
    f_conf.b_sub_f = b_sub_f;
    ...
    for (i = 0; i < m / sub_m; i++) {
        for (j = 0; j < m / sub_m; j++) {
            ...
            for (k = 0; k < m / sub_m; k++) {
                cudaMemcpyFromMmap(&f_conf, id, k, i, SUB_M, 1, 0, (char *) a_sub_f, (char *) res->buf, dsize * sizeof(float));
                cudaMemcpyFromMmap(&f_conf, id, j, k, SUB_M, 1, 1, (char *) b_sub_f, (char *) res->buf, dsize * sizeof(float));            
