...
__global__ void d2f_kernel(const float *din, float *dout, size_t dsize) {
...
int spdk_nds_blockSgemm_half_pthread(struct resources *res, uint64_t id, uint64_t m, uint64_t sub_m, float *c) {
    ...
    cudaMalloc((void **) &a_sub_f, sizeof(float) * dsize);
    cudaMalloc((void **) &b_sub_f, sizeof(float) * dsize);
    ...
    for (i = 0; i < m / sub_m; i++) {
        for (j = 0; j < m / sub_m; j++) {
            ...
            for (k = 0; k < m / sub_m; k++) {
                cudaMemcpyFromMmap(&f_conf, id, k, i, SUB_M, 1, 0, (char *) a_sub_d, (char *) res->buf, dsize * sizeof(float));
                cudaMemcpyFromMmap(&f_conf, id, j, k, SUB_M, 1, 1, (char *) b_sub_d, (char *) res->buf, dsize * sizeof(float));            

                timing_info_push_start(d2h_timing);
                d2f_kernel<<<(dsize+MAX_THREAD-1)/MAX_THREAD,MAX_THREAD>>>(a_sub_d, a_sub_f, dsize);
                d2f_kernel<<<(dsize+MAX_THREAD-1)/MAX_THREAD,MAX_THREAD>>>(b_sub_d, b_sub_f, dsize);
                ...
                for (sub_i = 0; sub_i < SUB_M; sub_i+=SUBSUB_M) {
                    for (sub_j = 0; sub_j < SUB_M; sub_j+=SUBSUB_M) {
                        for (sub_k = 0; sub_k < SUB_M; sub_k+=SUBSUB_M) {
                            cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, SUBSUB_M, SUBSUB_M, SUBSUB_M, &alpha, (b_sub_f + sub_k * SUB_M + sub_i), CUDA_R_32F, SUBSUB_M, (a_sub_f + sub_i * SUB_M + sub_k), CUDA_R_32F, SUBSUB_M, &beta, c_sub_f + sub_i * SUB_M + sub_j, CUDA_R_32F, SUBSUB_M, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
                        }
                    }
                }
