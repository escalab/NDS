...
uint64_t row_verify(double *arr, double *answer, uint64_t st, uint64_t m, uint64_t sub_m) {
...
int nds_aggregated_verify(struct resources *res, uint64_t id, uint64_t m, uint64_t sub_m, double *answer) {
    ...
    dsize = M * SUB_M * sizeof(double);
    stripe_matrix = (double *) malloc(dsize);
    memset(stripe_matrix, 0, dsize);
    ...
    for (st = 0; st < M / SUB_M; st++) {
        ...
        memcpy(stripe_matrix, res->buf + entry->resp->offset, dsize);

        // rc = rc | aggregated_verify(stripe_matrix, answer, j, i, m, sub_m);
        error += row_verify(stripe_matrix, answer, st * sub_m, m, sub_m);
        printf("result: %lu\n", error);
        free(entry->resp);
