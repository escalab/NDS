...
__global__ void pagerank_update(double* prev_pr, double* curr_pr, double *vertices, size_t st, int64_t* inedges, int64_t *outedges, int m, int sub_m, int iter, int niters) {
    ...
    // needs: v.num_inedges(), v.inedge(), v.out_id(), v.outc, v.set_data
    size_t out_id = threadIdx.x + blockDim.x * blockIdx.x;
    
    if (out_id >= sub_m) {
        return;
    }

    int64_t *outedge = outedges + out_id * m;
    int64_t *inedge = inedges + out_id;
    int i, outc = 0;
    double sum = 0;

    out_id = st + out_id;
    for (i = 0; i < m; i++) {
        if (i != out_id && outedge[i] != 0) {
            outc++;
        }
    }
    ...
    if (iter > 0) {
        for (i = 0; i < m; i++) {
            ...
            if (inedge[i * sub_m] && i != out_id) {
                sum += prev_pr[i];
            }
        }
        if (outc > 0) {
            curr_pr[out_id] = (RANDOMRESETPROB + (1 - RANDOMRESETPROB) * sum) / (double) outc;
        } else {
            curr_pr[out_id] = (RANDOMRESETPROB + (1 - RANDOMRESETPROB) * sum);
        }
    } else if (iter == 0) {
        if (outc > 0) {
            curr_pr[out_id] = 1.0f / (double) outc;
        }
    }
    ...
    if (iter == niters - 1) {
        if (outc > 0) {
            vertices[out_id] = curr_pr[out_id] * (double) outc;
        } else {
            vertices[out_id] = curr_pr[out_id];
        }
    }
}
...
void *request_thread(void *args) {
    ...
    for (i = 0; i < NITERS; i++) {
        for (st = 0; st < M / SUB_M; st++) {
            // row, out-edge
            sock_write_request(conf->res->req_sock, conf->out_id, st, st+1, SUB_M, 2, 0);
            sock_read_data(conf->res->req_sock);

            // col, in-edge
            sock_write_request(conf->res->req_sock, conf->in_id, st, st+1, SUB_M, 3, 1);
            sock_read_data(conf->res->req_sock);
        }
    }
...
int nds_pagerank(struct resources *res, uint64_t out_id, uint64_t in_id, uint64_t m, uint64_t sub_m) {
    size_t i, st;    
    int64_t *outedges, *inedges;
...
int main(int argc, char *argv[]) {
    int rc = 0;
    uint64_t out_matrix_id, in_matrix_id, n, sub_n;
    ...
    if (argc < 6) {
        printf("usage: %s <out_matrix_id> <in_matrix_id> <# of vertices> <# of subvertices> <port>\n", argv[0]);
        exit(1);
    } 
    out_matrix_id = (uint64_t) atoll(argv[1]);
    in_matrix_id = (uint64_t) atoll(argv[2]);
    ...
    rc = nds_pagerank(&res, out_matrix_id, in_matrix_id, n, sub_n);
