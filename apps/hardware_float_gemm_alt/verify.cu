...
__global__ void d2h_kernel(const double *din, float *dout, size_t dsize) {
...
int spdk_nds_blockSgemm_half_pthread(struct resources *res, uint64_t id, uint64_t m, uint64_t sub_m, float *c) {
    ...
    // cuda malloc
    dsize = sub_m * sub_m;
    b = (float *) malloc(sizeof(float) * dsize);
    ...
    for (k = 0; k < m / sub_m; k++) {
        for (i = 0; i < m / sub_m; i++) {
            ...
            d2h_kernel<<<(dsize+MAX_THREAD-1)/MAX_THREAD,MAX_THREAD>>>(entry->a_sub_d, a_sub_f, dsize);
            fifo_push(complete_queue, entry);
            timing_info_push_end(d2h_timing);
            for (j = 0; j < m / sub_m; j++) {
                // copy and transform B
                timing_info_push_start(b_copy_in_timing);
                cudaMemcpy(b_sub_f, b, sub_m * sub_m * sizeof(float), cudaMemcpyHostToDevice);
                ...
                cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, sub_m, sub_m, sub_m, &alpha, b_sub_f, CUDA_R_32F, sub_m, a_sub_f, CUDA_R_32F, sub_m, &beta, c_sub_f, CUDA_R_32F, sub_m, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
