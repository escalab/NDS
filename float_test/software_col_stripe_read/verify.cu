...
#define AGGREGATED_SZ (M * SUB_M * 4UL)
#define NITERS 4
...
int nds_col_stripe_read(struct resources *res, uint64_t id) {
    ...
    cudaMalloc(&d_addr, M * SUB_M * sizeof(float));
    ...
    for (i = 0; i < NITERS; i++) {
        for (st = 0; st < (M / SUB_M); st++) {
            ...
            cudaMemcpy(d_addr, res->buf + entry->resp->offset, dsize * sizeof(float), cudaMemcpyHostToDevice);
            free(entry->resp);
