...
int spdk_nds_blockSgemm_half_pthread(struct resources *res, uint64_t id, uint64_t m, uint64_t sub_m, float *c) {
    ...
    cudaMalloc((void **)&d_Input,   dsize * sizeof(float));
    cudaMalloc((void **)&d_Output,  dsize * sizeof(float));
    cudaMalloc((void **)&d_Buffer , dsize * sizeof(float));
    ...
    for (i = 0; i < m / sub_m; i++) {
        for (j = 0; j < m / sub_m; j++) {
            ...
            cudaMemset(d_Output, 0, dsize * sizeof(float));
            ...
            cudaMemcpy(d_Input, res->buf + entry->resp->offset, dsize * sizeof(float), cudaMemcpyHostToDevice);
            free(entry->resp);
            ...
            cudaMemcpy(c_ptr, d_Output, dsize * sizeof(float), cudaMemcpyDeviceToHost);
            c_ptr += dsize;
