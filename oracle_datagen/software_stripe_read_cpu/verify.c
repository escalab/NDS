...
void *fetch_thread(void *args) {
    ...
    for (st = 0; st < conf->m / conf->sub_m; st++) {
        ...
        timing_info_push_start(conf->stripe_fetch_timing);
        entry->resp = sock_read_offset(conf->res->sock);
        if (entry->resp == NULL) {
            fprintf(stderr, "sync error before RDMA ops\n");
            return NULL;
        }
        memcpy(d_addr, conf->res->buf + entry->resp->offset, M * SUB_M * sizeof(double));
        // msync(conf->out_addr, M * M * sizeof(double), MS_SYNC);
        d_addr += M * SUB_M * sizeof(double);
        if (sock_write_data(conf->res->sock)) { /* just send a dummy char back and forth */
            fprintf(stderr, "sync error before RDMA ops\n");
            return NULL;
        }
        timing_info_push_end(conf->stripe_fetch_timing);
...
void *request_thread(void *args) {
    ...
    for (st = 0; st < M / SUB_M; st++) {
        sock_write_request(conf->res->req_sock, conf->id, st, st+1, conf->sub_m, CURRENT_OP, 0);
        sock_read_data(conf->res->req_sock);
    }
    // send a signal to tell storage backend the iteration is done.
    sock_write_request(conf->res->req_sock, -1, st, st+1, SUB_M, CURRENT_OP, 0);
    sock_read_data(conf->res->req_sock);
    return NULL;
}

int nds_stripe_oracle_datagen(struct resources *res, uint64_t id, char *out_addr) {
    size_t i, st;
    ...
    size_t total_iteration = (M / SUB_M);
...
int main(int argc, char *argv[]) {
    ...
    rc = nds_stripe_oracle_datagen(&res, matrix_id, out_addr);
    ...
    msync(out_addr, dsize, MS_SYNC);
    munmap(out_addr, dsize);
    close(out_fd);
